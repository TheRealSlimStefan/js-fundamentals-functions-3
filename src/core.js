...
function timerStatus(remainingMinutes) {
  ...
  else if (remainingMinutes > 0) return 'The cake is still baking!'
...
function estimatePrepTime(ingredients, prepTime) {
  if (prepTime === undefined) return ingredients.length * 2
...
function calculateQuantities(ingredients, layers) {
  ...
  if (ingredients.includes('sugar') && ingredients.includes('eggs')) {
    object.eggs = layers * 2
    object.sugar = layers * 100
